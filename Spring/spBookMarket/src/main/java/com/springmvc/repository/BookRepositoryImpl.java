...
public class BookRepositoryImpl implements BookRepository {
	...
	public BookRepositoryImpl() {
		...
		book1.setAuthor("박용준");
		book1.setDescription("C# 교과서는 생애 첫 프로그래밍 언어로 C#을 시작하는 독자를 대상으로 한다. 특히 응용"
				+ " 프로그래머를 위한 C# 입문서로, C#을 사용하여 게임(유니티), 웹, 모바일, IoT 등을 개"
				+ "발할 때 필요한 C# 기초 문법을 익히고 기본기를 탄탄하게 다지는 것이 목적이다.");
		...
		book2.setDescription("이 책은 프런트부터 서버, 데이터베이스, 배포까지 아우르는 광범위한 내용을 다룬다. "
				+ "군더더기 없는 직관적인 설명으로 기본 개념을 확실히 이해하고, 노드의 기능"
				+ "과 생태계를 사용해 보면서 실제로 동작하는 서버를 만들어보자ㅏ. 예제와 코드는 최"
				+ "신 문법을 사용했고 실무에 참고하거나 당장 적용할 수 있다.");
		...
		book3.setDescription("어도비 XD 프로그램을 통해 UI/UX 디자인을 배우고자 하는 예비 디자이너의 눈높이에 맞게 "
				+ "기본적인 도구를 활용한 아이콘 디자인과 웹&앱 페이지 디자인, UI 디자인, "
				+ "앱 디자인에 애니메이션과 인터렉션을 적용한 프로토타이핑을 학습합니다.");
	...
	public void setUpdateBook(Book book) {
		if(book.getFileName() != null) {
			String SQL = "UPDATE Book SET b_name=?, b_unitPrice=?, b_author=?, b_description=?, b_publisher=?, b_category=?, "
					+ "b_unitsInStock=?, b_releaseDate=?, b_condition=?, b_fileName=? where b_bookId=?";
			...
			String SQL = "UPDATE Book SET b_name=?, b_unitPrice=?, b_author=?, b_description=?, b_publisher=?, b_category=?, "
					+ "b_unitsInStock=?, b_releaseDate=?, b_condition=? where b_bookId=?";
			template.update(SQL, book.getName(), book.getUnitPrice(), book.getAuthor(), book.getDescription(), book.getPublisher(), book.getCategory(),
